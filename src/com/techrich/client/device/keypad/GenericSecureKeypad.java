...
//10058F9F1418F590
public class GenericSecureKeypad extends SecureKeypad {
	...
	public GenericSecureKeypad() {
		this.deviceId = "SECURE-KEYPAD-NORMAL";
	}
	
	@Override
	public String getDeviceName() {
		return deviceName!=null?deviceName:"通用密码键盘";
	}
	...
	public void initDevice() throws DeviceInitException {
		...
		try{
			...
			if(version.startsWith("STC2"))newKMY = true;
...
//		try{
//			comm2keypad(new byte[]{0x49});//复位所有IC卡
//		}catch(Exception e){
...
//		selectSocket(KEYPAD_SAM_SOCKET, false);//选择
//		comm2keypad(new byte[]{0x5B});//对IC卡断电
...
//			String macKeyCv = Tools.bytes2hex(bb.getValueN(len));
			...
			LogManager.logInfo("PINKEY解密前[:"+Tools.bytes2hex(macKey)+"]后["+Tools.bytes2hex(macKeyResult)+"]");
		}
