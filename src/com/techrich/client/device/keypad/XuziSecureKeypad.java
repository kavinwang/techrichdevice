...
public class XuziSecureKeypad extends SecureKeypad {
	private static final int CMD_SUCCESS = 0x04;
	private static final int PROTOCOL_HEADER = 0x02;

	private static final byte SAM_MKEY = 0x0F;//旭子以0X0F为SAM卡的主密钥
	private static final byte SAM_PINKEY = 0x00;//旭子以0X00为SAM卡的保护下工作密钥PIK
	private static final byte SAM_MACKEY = 0x01;//旭子以0X01为SAM卡的保护下工作密钥MAK
	
	boolean logComm = true;
	public XuziSecureKeypad(){
		this.deviceId = "SECURE-KEYPAD-XUZI";
	}
	@Override
	public String getDeviceName() {
		return deviceName!=null?deviceName:"深圳旭子密码键盘";
	}
	...
	public void initDevice() throws DeviceInitException {
		...
		try{
			...
			if(version.startsWith("YD256-V11 V1.07")){
				LogManager.logInfo("密码键盘固件版本："+version+" 本版本支持3DES");
				tdesEnabled = true;
			}else{
				LogManager.logInfo("密码键盘固件版本："+version+" 本版本不支持3DES");
				tdesEnabled = false;
			}
...
//			String macKeyCv = Tools.bytes2hex(bb.getValueN(8));
			...
			ByteBuffer kb = new ByteBuffer(new byte[]{0x4A, SAM_MKEY, SAM_MACKEY});
...
//		if(timeOut>0)try{Thread.sleep(timeOut);}catch(Exception ee){};
	...
	public void selectSocket(int socket, boolean poweron) throws Exception {
		...
		if(poweron){
			LogManager.logInfo("ATR:"+Tools.bytes2hex(comm2keypad(new byte[]{0x49})));
		}
	}
